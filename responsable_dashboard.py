...
class ResponsableDashboard:
   ...
   #-----Historique---
    def create_history_tab(self):
        ...
        self.history_selected_vehicle_id = tk.StringVar()
        self.history_vehicle_entry = ttk.Entry(filter_frame, textvariable=self.history_selected_vehicle_id, font=self.font_label)
        self.history_vehicle_entry.grid(row=0, column=1, padx=5, pady=2, sticky="ew")
        ...
        tk.Button(filter_buttons_frame, text="Réinitialiser Filtres", command=self.clear_history_filters, font=self.font_button_text, bg="#7F8C8D", fg=self.button_fg).pack(side="left", padx=5, expand=True)
        ...
        self.show_detailed_history_button = tk.Button(
            ...
            text="Afficher Historique Détaillé",
        ...
        self.history_filter_date_obj = None # Initialize this attribute
    ...
    def clear_history_filters(self):
        self.history_selected_vehicle_id.set("")
    ...
    def show_single_vehicle_history(self):
        ...
        if not selected_veh_id_val or not selected_veh_id_val.isdigit():
        ...
        user_map = {int(u[0]): f"{u[4]} {u[3]}" for u in get_all_chauffeurs_and_responsables()}

        attributions = get_all_attributions()
        vehicle_attributions = sorted([a for a in attributions if len(a) > 2 and isinstance(a[2], (int, str)) and int(a[2]) == selected_veh_id_int], 
        ...
        if vehicle_attributions:
            ...
            for attr in vehicle_attributions:
                chauffeur_name = user_map.get(int(attr[1]), "Chauffeur inconnu")
        ...
        maintenances = get_all_maintenances()
        vehicle_maintenances = sorted([m for m in maintenances if len(m) > 10 and isinstance(m[10], (int, str)) and int(m[10]) == selected_veh_id_int],
        ...
        fuel_entries = get_all_fuel_entries()
        vehicle_fuel_entries = sorted([f for f in fuel_entries if len(f) > 13 and isinstance(f[13], (int, str)) and int(f[13]) == selected_veh_id_int],
                                      key=lambda x: self._safe_parse_date(x[4]), reverse=True)
        if vehicle_fuel_entries:
            self.single_vehicle_history_text.insert(tk.END, "CARBURANT:\n")
            for entry in vehicle_fuel_entries:
                user_name = user_map.get(int(entry[12]), "Inconnu")
        ...
        incident_reports = get_all_incident_reports()
        vehicle_incident_reports = sorted([i for i in incident_reports if len(i) > 2 and isinstance(i[2], (int, str)) and int(i[2]) == selected_veh_id_int],
                                         key=lambda x: self._safe_parse_date(x[3]), reverse=True)
        if vehicle_incident_reports:
            ...
            for report in vehicle_incident_reports:
                user_name = user_map.get(int(report[1]), "Inconnu")
        ...
        vehicle_inspection_reports = sorted([i for i in inspection_reports if len(i) > 2 and isinstance(i[2], (int, str)) and int(i[2]) == selected_veh_id_int],
                                             key=lambda x: self._safe_parse_date(x[3]), reverse=True)
    ...
    def populate_history_treeview(self, vehicle_id=None, event_type=None, filter_date=None):
        ...
        vehicle_map = {int(v[0]): v[1] for v in get_all_vehicles()}
        user_map = {int(u[0]): f"{u[4]} {u[3]} ({u[1]})" for u in get_all_chauffeurs_and_responsables()} # Added username for differentiation
        ...
        if event_type is None or event_type == "Attribution":
            ...
            for attr in attributions:
                if len(attr) < 13:
                  print(f"Alerte: Ligne d'attribution incomplète (ID {attr[0] if len(attr)>0 else 'N/A'}). Ignorée.")
                  continue 

                 # CHANGE THIS LINE: Remove the int() conversion
                 # If attr[2] is the vehicle registration (e.g., '2235ABC'), keep it as a string.
                attr_db_vehicle_id = attr[2] 
        
                attr_db_chauffeur_id = int(attr[1]) if isinstance(attr[1], (int, str)) else None
                attr_date_start = attr[4]
                formatted_date_start = self._format_date_for_treeview(attr_date_start)

                if (filter_vehicle_id_int is None or (attr_db_vehicle_id is not None and attr_db_vehicle_id == filter_vehicle_id_int)) and \
                   (filter_date is None or formatted_date_start == filter_date.strftime("%Y-%m-%d")):
            
                   chauffeur_nom_complet = user_map.get(attr_db_chauffeur_id, "Chauffeur inconnu")
            
                # CHANGE THIS LINE: vehicle_map must now use the registration string as its key
            ...
            # Maintenances (omitted for brevity, similar structure to previous example)
            # Carburant (omitted for brevity)
           # Incidents (omitted for brevity)
           # Inspections (omitted for brevity)
        ...
        if self.single_vehicle_history_text.winfo_ismapped(): 
            ...
            if current_vehicle_id_filter and current_vehicle_id_filter != "Tous":
                    try:
                        ...
                        filter_parts.append(f"Vehicule_ID_Invalid_{current_vehicle_id_filter.replace(' ', '_')}")
            ...
            filename_prefix = "_".join(filter_parts) if filter_parts else "Historique_Global_Tableau"
...
# --- NEW STATISTICS TAB CODE ---
    def create_statistics_tab(self):
        ...
        self.stat_vehicle_combobox.bind("<<ComboboxSelected>>", self.populate_statistics_dashboard) # Update on selection

        tk.Label(filter_frame, text="Période (Début):", bg=self.bg_color_dashboard, fg=self.fg_color_text, font=self.font_label).grid(row=1, column=0, padx=5, pady=2, sticky="w")
        self.stat_start_date_entry = DateEntry(filter_frame, width=12, background="darkblue", foreground="white", borderwidth=2,
                                                 font=self.font_label, date_pattern='yyyy-mm-dd')
        self.stat_start_date_entry.grid(row=1, column=1, padx=5, pady=2, sticky="ew")
        self.stat_start_date_entry.set_date(None)

        tk.Label(filter_frame, text="Période (Fin):", bg=self.bg_color_dashboard, fg=self.fg_color_text, font=self.font_label).grid(row=2, column=0, padx=5, pady=2, sticky="w")
        self.stat_end_date_entry = DateEntry(filter_frame, width=12, background="darkblue", foreground="white", borderwidth=2,
                                               font=self.font_label, date_pattern='yyyy-mm-dd')
        self.stat_end_date_entry.grid(row=2, column=1, padx=5, pady=2, sticky="ew")
        ...
        self.stats_display_frame.grid_rowconfigure(0, weight=1)

        # Use a Text widget for displaying detailed statistics
        self.statistics_text_output = tk.Text(self.stats_display_frame, wrap="word", font=self.font_label,
                                             bg=self.entry_bg, fg=self.fg_color_text, insertbackground=self.fg_color_text)
        self.statistics_text_output.pack(expand=True, fill="both")
        self.statistics_text_output.config(state=tk.DISABLED) # Make it read-only

    def _get_vehicle_options_for_stats(self):
        """Gets vehicle options for the combobox, including 'Tous'."""
        vehicles = get_all_vehicles()
        options = ["Tous"]
        for veh in vehicles:
            options.append(f"{veh[0]} - {veh[1]}") # e.g., "1 - AB-123-CD"
        return options

    def clear_statistics_filters(self):
        """Clears the filters on the statistics tab."""
    ...
    def populate_statistics_dashboard(self, event=None):
        ...
        filtered_attributions = [
            attr for attr in all_attributions
            if (target_vehicle_id is None or (len(attr) > 2 and isinstance(attr[2], (int, str)) and int(attr[2]) == target_vehicle_id))
            and self._is_date_in_range(self._safe_parse_date(attr[4]).date(), filter_start_date, filter_end_date)
        ]

        filtered_maintenances = [
            maint for maint in all_maintenances
            if (target_vehicle_id is None or (len(maint) > 10 and isinstance(maint[10], (int, str)) and int(maint[10]) == target_vehicle_id))
            and self._is_date_in_range(self._safe_parse_date(maint[3]).date(), filter_start_date, filter_end_date)
        ]

        filtered_fuel_entries = [
            fuel for fuel in all_fuel_entries
            if (target_vehicle_id is None or (len(fuel) > 13 and isinstance(fuel[13], (int, str)) and int(fuel[13]) == target_vehicle_id))
            and self._is_date_in_range(self._safe_parse_date(fuel[4]).date(), filter_start_date, filter_end_date)
        ]

        filtered_incident_reports = [
            inc for inc in all_incident_reports
            if (target_vehicle_id is None or (len(inc) > 2 and isinstance(inc[2], (int, str)) and int(inc[2]) == target_vehicle_id))
            and self._is_date_in_range(self._safe_parse_date(inc[3]).date(), filter_start_date, filter_end_date)
        ]
        ...
        for attr in filtered_attributions:
            if len(attr) > 8 and attr[7] is not None and attr[8] is not None:
                try:
                    ...
                    pass # Handle cases where km might not be numbers

        total_mileage_from_fuel = 0
        last_odometer_reading = {} # To calculate mileage between consecutive fuel entries for a vehicle
        for fuel_entry in sorted(filtered_fuel_entries, key=lambda x: self._safe_parse_date(x[4])):
            veh_id = int(fuel_entry[13])
            current_odometer = fuel_entry[9]
        ...
        total_fuel_cost = sum(entry[7] for entry in filtered_fuel_entries if len(entry) > 7 and entry[7] is not None)
        total_fuel_quantity = sum(entry[6] for entry in filtered_fuel_entries if len(entry) > 6 and entry[6] is not None)
        ...
        total_maintenance_cost = sum(maint[4] for maint in filtered_maintenances if len(maint) > 4 and maint[4] is not None)
    ...
    def _is_date_in_range(self, event_date, start_date, end_date):
        """Helper to check if an event date falls within the selected date range."""
        if not event_date: # If the event has no date, it's considered outside any specific filter
            return False
